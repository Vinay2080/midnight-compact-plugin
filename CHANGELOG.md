...
### Added
- Added automatic angle bracket (`<>`) pairing, cursor placement (`<|>`), overtyping step-over on `>`, and paired backspace deletion for generic types, declarations, and parameterized expressions via `CompactAngleBraceTypedHandler` and `CompactAngleBraceBackspaceHandler`.
...
### Fixed
- Fixed missing angle bracket (`<>`) auto-insertion and mirroring when typing `<` after generic types, parameterized expressions, and declaration headers.
